...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
      <header className="pointer-events-auto flex items-center justify-between w-full max-w-5xl h-16 px-6 rounded-full glass-panel shadow-2xl backdrop-blur-2xl">
        {/* Brand Logo */}
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="relative flex h-9 w-9 items-center justify-center rounded-full bg-white/5 border border-white/10 group-hover:border-accent-primary/40 transition-colors">
            <Orbit className="h-5 w-5 text-accent-primary animate-orbit-fast" />
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[9px] text-gray-500 tracking-wider uppercase font-semibold">
              ISRO HACKATHON
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center">
          <Link
            href="/dashboard"
            className="inline-flex items-center justify-center px-5 py-2 text-[13px] font-bold tracking-wide text-bg-primary bg-accent-primary rounded-full hover:shadow-lg hover:shadow-accent-primary/30 hover:-translate-y-0.5 active:translate-y-0 active:scale-95 transition-all duration-200"
          >
            LAUNCH WORKSPACE
          </Link>
        </div>
